...
! function(win) {
	...
	Dates.viewDate = function(Y, M, D) {
		...
		Dates.each(as.tds, function(i, elem) {
			...
			Dates.options.isFestival && Dates.isFestival(elem, MM + '.' + DD);
	...
	//节日
	Dates.isFestival = function(td, md) {
		var str;
		switch (md) {
			case '1.1':
				str = '元旦';
	...
	Dates.viewYears = function(YY) {
		var S = Dates.query,
			str = '';
		Dates.each(new Array(10), function(i) {
			if (i === 7) {
	...
	Dates.view = function(elem, options) {
		...
		if (!Dates.box) {
			...
			div.innerHTML = log.html = '<div class="laydate_top">' + '<div class="laydate_ym laydate_y" id="laydate_YY">' + '<a class="laydate_choose laydate_chprev laydate_tab">&lt;&lt;</a>' + '<input id="laydate_y" readonly><label></label>' + '<a class="laydate_choose laydate_chnext laydate_tab">&gt;&gt;</a>' + '<div class="laydate_yms">' + '<a class="laydate_tab laydate_chtop">&lt;</a>' + '<ul id="laydate_ys"></ul>' + '<a class="laydate_tab laydate_chdown">&gt;</a>' + '</div>' + '</div>' + '<div class="laydate_ym laydate_m" id="laydate_MM">' + '<a class="laydate_choose laydate_chprev laydate_tab">&lt;</a>' + '<input id="laydate_m" readonly><label></label>' + '<a class="laydate_choose laydate_chnext laydate_tab">&gt;</a>' + '<div class="laydate_yms" id="laydate_ms">' + function() {
				var str = '';
			...
			+ Dates.viewtb
				...
				+ '<div class="laydate_bottom">' + '<ul id="laydate_hms">' + '<li><input readonly><span class="laydate-time-format">:</i></li>' + '<li><input readonly><span class="laydate-time-format">:</i></li>' + '<li><input readonly></li>' + '</ul>' + '<div class="laydate_time" id="laydate_time"></div>' + '<div class="laydate_btn">' + '<a id="laydate_clear">清空</a>'
				/*  +'<a id="laydate_today">今天</a>'*/
				+ '<a id="laydate_ok">确认</a>' + '</div>' + (config.isv ? '<a href="http://sentsin.com/layui/laydate/" class="laydate_v" target="_blank">laydate-v' + laydate.v + '</a>' : '') + '</div>';
	...
	Dates.events = function() {
		...
		/*Dates.each(S('#laydate_YM .laydate_tab_Y'), function(i, elem){
		    Dates.on(elem, 'click', function(ev){
		        Dates.stopmp(ev);
		        log.tabYear(i);
		    });
		...
		/* Dates.each(S('#laydate_YM .laydate_tab_M'), function(i, elem){
		     Dates.on(elem, 'click', function(ev){
		         Dates.stopmp(ev).reshow();
		         log.tabMonth(i);
		     });
...
(function($) {
	$.fn.datePicker = function(options) {
		...
		if(options.format){
			if(options.istime){
				str = options.format || 'YYYY-MM-DD hh:mm:ss';
			}else{
				str = options.format.substring(0,10) || 'YYYY-MM-DD';
			}
		}else{
			if(options.istime){
